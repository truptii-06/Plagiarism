...
const DatasetManagement = () => {
  ...
  const fetchDatasets = async () => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/datasets?teacherId=${teacherId}`,
      );
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await fetch(
        '${import.meta.env.VITE_API_URL}/api/datasets/upload',
        {
          method: 'POST',
          body: formData,
        },
      ...
      if (data.success) {
        setMessage({
          type: 'success',
          text: 'Dataset uploaded and processed successfully!',
        });
        setName('');
        setFile(null);
        fetchDatasets();
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        'Are you sure you want to delete this dataset? This will remove all records used for plagiarism comparison.',
      )
    )
      return;
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/datasets/${id}`,
        {
          method: 'DELETE',
        },
      );
  ...
  return (
    <div className="dataset-mgmt-container">
      <div className="dataset-header">
        ...
        <p>
          Upload CSV or Excel files containing previous students' work or
          reference material to compare against new submissions.
        </p>
      </div>
        ...
        <div className="list-section panel">
          ...
          {loading ? (
            <div className="loading-state">
              <Loader2 className="spinner" /> Loading datasets...
            </div>
          ) : datasets.length === 0 ? (
