        ...
        <form onSubmit={saveProfile} className="profile-form">
          ...
          <div className="buttons-row">
            <button
              ...
              className="small-btn"
              onClick={() =>
                updateProfileField("groupMembers", [...profile.groupMembers, ""])
              }
            >
              + Add Member
            </button>
          ...
          <tbody>
            {submissions.map((s) => (
              <tr key={s._id}>
                ...
                <td>
                  <button type="button" className="download-link" onClick={() => handleDownload(s.fileUrl)}>
                    <Download size={14} /> View
                  </button>
                </td>
                <td>{s.teacherFeedback || "-"}</td>
                <td>
                  {["Rejected", "Needs Correction"].includes(s.status) ? (
                    <button type="button" className="small-btn" onClick={() => handleResubmit(s._id)}>
                      Re-submit
                    </button>
                  ) : (
    ...
    <div className="student-dashboard">
      ...
      <main className="main-content">
        ...
        <div className="dashboard-content-area">
          ...
          {activePage === "submissions" && <SubmissionsContent />}
        </div>
      </main>
    </div>
  );
